...
export default function DepositPage() {
  ...
  // Fetch deposit address when component mounts or coin changes
  useEffect(() => {
    loadDepositAddress();
  }, [selectedCoin.symbol]);
  ...
  const loadDepositAddress = async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('coin_symbol', selectedCoin.symbol);
      ...
      if (data && data.length > 0 && data[0]?.wallet_address) {
        setDepositAddress(data[0].wallet_address);
      } else {
        setDepositAddress('');
      }
    } catch (err) {
      console.error('Exception loading address:', err);
      setDepositAddress('');
    }
  };
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          readOnly
                          placeholder={depositAddress ? '' : `Loading ${selectedCoin.symbol} address...`}
