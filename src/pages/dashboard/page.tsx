...
export default function DashboardPage() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      // Load user's profile for account balances
      ...
      // Add account balances (USD, BTC, ETH, etc.)
      if (profileData) {
        total += profileData.usd_balance || 0;
        total += (profileData.btc_balance || 0) * 50000; // Approximate BTC price
        total += (profileData.eth_balance || 0) * 3000;  // Approximate ETH price
        // Add other account balances as needed
      }

      // Add portfolio holdings value
      if (portfolioData && portfolioData.length > 0) {
        const coinData = await cryptoApi.getTopCoins(50);
        
        for (const holding of portfolioData) {
          const coin = coinData.find(c => c.symbol.toLowerCase() === holding.coin_symbol.toLowerCase());
