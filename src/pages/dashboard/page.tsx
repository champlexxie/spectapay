...
export default function DashboardPage() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      // Fetch portfolio, wallets, transactions, and market data in parallel
      const [portfolioResult, walletResult, transactionsResult, marketData] = await Promise.all([
        supabase
          .from('portfolios')
          .select('*')
          .eq('user_id', user.id)
      ...
      // Combine portfolio and wallet data
      const allHoldings = new Map<string, { amount: number; name: string }>();

      // Add portfolio data
      if (portfolioResult.data && portfolioResult.data.length > 0) {
        portfolioResult.data.forEach(item => {
          const symbol = item.coin_symbol?.toUpperCase();
          if (symbol) {
            allHoldings.set(symbol, {
              amount: Number(item.amount) || 0,
              name: item.coin_name || symbol
            });
          }
        });
      }

      // Add wallet data (merge with portfolio if exists)
      if (walletResult.data && walletResult.data.length > 0) {
        walletResult.data.forEach(wallet => {
          const symbol = wallet.coin_symbol?.toUpperCase() || wallet.currency?.toUpperCase();
          if (symbol) {
            const existing = allHoldings.get(symbol);
            const walletBalance = Number(wallet.balance) || 0;
            
            if (existing) {
              existing.amount += walletBalance;
            } else {
              allHoldings.set(symbol, {
                amount: walletBalance,
                name: wallet.coin_name || symbol
              });
            }
          }
        });
      ...
      // Calculate balances with current market prices
      if (allHoldings.size > 0) {
        const portfolioWithPrices = Array.from(allHoldings.entries()).map(([symbol, data]) => {
          // Get current price from market data
          const marketCoin = marketData.find(c => c.symbol.toUpperCase() === symbol);
          const currentPrice = marketCoin ? marketCoin.current_price : (symbol === 'USDT' ? 1 : 0);
          ...
          return {
            ...
            change: marketCoin ? marketCoin.price_change_percentage_24h : 0,
            icon: marketCoin?.image || `https://coin-images.coingecko.com/coins/images/1/large/${symbol.toLowerCase()}.png`
          };
        });

        setPortfolioData(portfolioWithPrices);
        
        // Calculate total balance
        const total = portfolioWithPrices.reduce((sum, item) => sum + item.value, 0);
        setTotalBalance(total);
        setAvailableBalance(total);
      } else {
        // Only reset if we're sure there's no data
        if (!isRefreshing) {
          setTotalBalance(0);
