...
export default function DashboardPage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Fetch portfolios, transactions, wallets, and KYC status in parallel
      const [portfolioResult, transactionResult, walletResult, kycResult] = await Promise.all([
        supabase
          .from('portfolios')
          .select('*')
          .eq('user_id', user.id),
      ...
      if (walletData && walletData.length > 0) {
        ...
        try {
          ...
          walletData.forEach(wallet => {
            ...
            totalUSD += wallet.balance * price;
          });

          setTotalBalance(totalUSD);
          setAvailableBalance(totalUSD);
        } catch (priceError) {
          console.warn('Price API unavailable, using fallback calculation');
          
          // Fallback: Use approximate prices for common cryptocurrencies
          ...
          walletData.forEach(wallet => {
            const currency = wallet.currency.toLowerCase();
            const coinId = coinMap[currency] || currency;
            const price = fallbackPrices[coinId] || 0;
            totalUSD += wallet.balance * price;
          });
