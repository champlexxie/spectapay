...
export default function WalletPage() {
  ...
  const fetchHoldings = useCallback(async () => {
    ...
    try {
      ...
      // Fetch balances from database with timeout
      const fetchPromise = supabase
        .from('wallets')
        .select('*')
        .eq('user_id', user.id);

      const { data: balances, error } = await Promise.race([
        fetchPromise,
        timeoutPromise
      ]) as any;

      if (error) {
        console.warn('Database fetch error, using default data:', error);
        throw error;
      }
      ...
      console.warn('Using default wallet data due to error:', error);
      // Always show default coins on error
      setHoldings(defaultCoins);
      setTotalBalance(0);
    } finally {
      setLoading(false);
