...
export default function WalletPage() {
  ...
  const fetchHoldings = useCallback(async () => {
    ...
    try {
      ...
      const holdingsMap = new Map<string, { total: number; name: string; logo: string; color: string }>();
      ...
      portfolioData.forEach(p => {
        const symbol = p.coin_symbol?.toUpperCase();
        if (symbol) {
          ...
          if (existing) {
            ...
            holdingsMap.set(symbol, {
              total: amount,
              name: p.coin_name || symbol,
              logo: `https://cryptologos.cc/logos/${symbol.toLowerCase()}-logo.svg`,
              color: 'bg-gray-500'
            });
          }
        }
      ...
      walletData.forEach(w => {
        const symbol = w.coin_symbol?.toUpperCase() || w.currency?.toUpperCase();
        if (symbol) {
          const existing = holdingsMap.get(symbol);
      ...
      const updatedHoldings: CoinHolding[] = Array.from(holdingsMap.entries()).map(([symbol, data]) => {
        ...
        const usdValue = data.total * currentPrice;
