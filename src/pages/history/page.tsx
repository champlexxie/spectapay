...
export default function HistoryPage() {
  ...
  const loadUserTransactions = async () => {
    ...
    try {
      ...
      // Load from transactions table (trading, deposits, withdrawals)
      ...
      if (tradingData) {
        tradingData.forEach(tx => {
          allTransactions.push({
            ...
            type: tx.transaction_type === 'buy' ? 'Buy' : 
                  tx.transaction_type === 'sell' ? 'Sell' :
                  tx.transaction_type === 'deposit' ? 'Deposit' : 'Withdraw',
            amount: parseFloat(tx.amount) || 0,
            asset: tx.coin_symbol?.toUpperCase() || 'USD',
            status: tx.status === 'completed' ? 'Completed' : 
                   tx.status === 'pending' ? 'Pending' : 'Failed',
            address: tx.wallet_address || tx.recipient_address || 'N/A',
      ...
      // Load from transfers table (external transfers)
      ...
      if (transferData) {
        transferData.forEach(tx => {
          ...
          allTransactions.push({
            ...
            type: isOutgoing ? 'Transfer (OUT)' : 'Transfer (IN)',
            amount: parseFloat(tx.amount) || 0,
            asset: tx.currency?.toUpperCase() || 'USD',
            status: tx.status === 'completed' ? 'Completed' : 
                   tx.status === 'pending' ? 'Pending' : 'Failed',
            address: isOutgoing ? tx.recipient_email : tx.sender_email,
            source: 'transfers'
          });
        });
      }

      // Load from internal_transfers table (internal transfers)
