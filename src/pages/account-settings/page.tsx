...
export default function AccountSettingsPage() {
  ...
  const handleFileUpload = async (documentType: string, file: File | null) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setKycError('File size must be less than 4MB');
      setTimeout(() => setKycError(''), 5000);
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (result.success) {
          setKycSuccess(result.message || 'Document uploaded successfully!');
        ...
        if (fetchError.name === 'AbortError') {
          throw new Error('Upload is taking longer than expected. Please try with a smaller file or check your connection.');
        }
        throw fetchError;
      }
    } catch (error: any) {
