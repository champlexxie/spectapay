...
export default function AccountSettingsPage() {
  ...
  const loadKycDocuments = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_PUBLIC_SUPABASE_URL}/functions/v1/kyc-upload`, {
        method: 'GET',
        headers: {
  ...
  const handleFileUpload = async (documentType: string, file: File) => {
    ...
    try {
      ...
      setKycError(error.message || 'Failed to upload document. Please try again.');
    } finally {
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {!kycDocuments.government_id || kycDocuments.government_id.status === 'rejected' ? (
                    <label className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap cursor-pointer">
                      {uploadingDoc === 'government_id' ? 'Uploading...' : 'Upload'}
                      <input
                        type="file"
                        accept="image/*,.pdf"
                        className="hidden"
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {!kycDocuments.proof_of_address || kycDocuments.proof_of_address.status === 'rejected' ? (
                    <label className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap cursor-pointer">
                      {uploadingDoc === 'proof_of_address' ? 'Uploading...' : 'Upload'}
                      <input
                        type="file"
                        accept="image/*,.pdf"
                        className="hidden"
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {!kycDocuments.selfie || kycDocuments.selfie.status === 'rejected' ? (
                    <label className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap cursor-pointer">
                      {uploadingDoc === 'selfie' ? 'Uploading...' : 'Upload'}
                      <input
                        type="file"
                        accept="image/*"
                        className="hidden"
