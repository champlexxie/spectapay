...
export default function DashboardPage() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      const [profileResponse, transactionResponse, portfolioResponse, walletResponse, priceResponse] = await Promise.all([
      ...
      if (profileResponse.data) {
        setKycStatus(profileResponse.data.kyc_status || null);
      }
      ...
      if (portfolioResponse.data) {
        ...
        portfolioResponse.data.forEach(holding => {
          if (holding.coin_symbol) {
      ...
      if (walletResponse.data) {
        walletResponse.data.forEach(wallet => {
          if (wallet.coin_symbol) {
            const symbol = wallet.coin_symbol.toUpperCase();
            const price = cryptoPrices[symbol as keyof typeof cryptoPrices] || 0;
            const walletBalance = (wallet.balance || 0) * price;
            total += walletBalance;
